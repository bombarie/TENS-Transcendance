...
void setIdActive(byte id) {
  Serial.print("f:setIdActive >> id: " + String(id));
...
void executeTriggersAtIndex(byte i) {
  Serial.print("executeTriggersAtIndex >> activating ");
...
void scheduleTrigger(int id, float pWidth, float pFreq, float pStrength, int duration) {
  id = constrain(id, 1, 16);

  if (!id_to_boardport[id].isSet) {
    Serial.println("scheduleDoubleTrigger >> id ");
    Serial.print(id);
...
void scheduleDoubleTrigger(
  ...
  id1 = constrain(id1, 1, 16);
  id2 = constrain(id2, 1, 16);
